 ...
 */
export function createDomainBlockGuard(
 ...
 */
export function createUserAgentBlockGuard(
  blockedUserAgents: string[],
): GuardCondition {
  return (context: GuardContext): boolean => {
    const userAgent = context.userAgent.toLowerCase();
    return blockedUserAgents.some((ua) => userAgent.includes(ua.toLowerCase()));
  };
}
 ...
 */
export function createQueryParamBlockGuard(
  paramName: string,
  value?: string,
): GuardCondition {
  return (context: GuardContext): boolean => {
 ...
 */
export function createLocalStorageGuard(
  key: string,
  value?: string,
): GuardCondition {
  return (_context: GuardContext): boolean => {
...
export function createCrawlerBotBlockGuard(): GuardCondition {
  ...
  return (context: GuardContext): boolean => {
    ...
    if (isLegitimateBrowser(context.userAgent)) {
      ...
      if (userAgent.includes('compatible;')) {
        ...
        const looksLikeRealBrowser =
          /(windows|macintosh|linux|x11|android|iphone|ipad)/i.test(
            compatiblePart,
          );
    ...
    if (genericBotPattern.test(userAgent)) {
      ...
      const hasBrowserLikePattern =
        /(chrome|firefox|safari|edge|opera|webkit|gecko)/i.test(userAgent);
    ...
    const hasAnyBrowserIndicator =
      /(mozilla|webkit|gecko|trident|presto)/i.test(userAgent);
    if (!hasAnyBrowserIndicator && userAgent.length > 0) {
      ...
      const botLikePattern = /^[a-z]+\/[0-9]+(\.[0-9]+)*\s*$/i.test(
        userAgent.trim(),
      );
      if (botLikePattern) {
