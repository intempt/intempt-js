...
export class PlatformParser {
  ...
  private readonly _platformVersions: Record<
    string,
  ...
  private detectDeviceType(): DeviceType {
    ...
    const isMediumScreen =
      screenWidth > mobileMaxScreenWidth && screenWidth <= tabletMaxScreenWidth;
  ...
  protected _getBrowser() {
    ...
    for (const { regex, exclude, result } of browsers) {
      if (
        regex.test(this._userAgent) &&
        (!exclude || !exclude.test(this._userAgent))
      ) {
        return result();
      }
    }
  ...
  protected async _handleUserAgentEntropyValue() {
    ...
    try {
      const highEntropyData = await navigator[
        'userAgentData'
      ]?.getHighEntropyValues(['platformVersion', 'platform']);
      if (
        highEntropyData &&
        highEntropyData?.platform &&
        highEntropyData?.platformVersion
      ) {
