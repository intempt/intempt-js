...
import {
  getReservedWordPatterns,
  shouldExcludeMatch,
} from '../config/reservedWordsPatterns.js';

describe('Bundle Reserved Words Check', () => {
  before(() => {
    // Build the bundle before running tests
    cy.exec('npm run build', { failOnNonZeroExit: false, timeout: 60000 }).then(
      (result) => {
  ...
  it('should not use reserved words as variable names in the minified bundle', () => {
    cy.readFile('dist/intempt.min.js', 'utf-8', { timeout: 10000 }).then(
      (bundleCode) => {
          ...
          patterns.forEach((pattern) => {
            ...
            if (matches) {
              matches.forEach((match: string) => {
                ...
                if (matchIndex !== -1) {
                  if (
                    !shouldExcludeMatch(word, match, matchIndex, bundleCode)
                  ) {
        ...
        if (violations.length > 0) {
          const uniqueViolations = Array.from(
            new Map(
              violations.map((v) => [`${v.word}:${v.match}`, v]),
            ).values(),
          );
