...
| **Blocked on localhost**                                     | Tracking is blocked on `localhost`, `*.localhost` and `127.0.0.1`, and for bot user agents. See [examples/README.md](../examples/README.md) for how to develop against this. `&allow_bots=1` on the script URL disables the bot guard only.                |
